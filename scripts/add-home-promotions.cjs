...
.avp-promotions-card{margin-top:13px}.avp-promotions-head{display:flex;align-items:flex-start;justify-content:space-between;gap:12px;margin-bottom:14px}.avp-promotions-head .section-title{margin:4px 0}.avp-promotions-grid{display:grid;grid-template-columns:1fr;gap:10px}.avp-promo{display:block;text-decoration:none;color:inherit;background:var(--panel2);border:1px solid var(--line);border-radius:14px;padding:14px;cursor:pointer;transition:transform .15s ease,border-color .15s ease}.avp-promo:active{transform:scale(.985)}.avp-promo.best{border-color:var(--green);box-shadow:0 0 0 1px #31d69b33}.avp-promo-route{font-size:18px;font-weight:900;margin:5px 0}.avp-promo-price{font-size:24px;font-weight:950;margin:8px 0}.avp-promo-meta{display:grid;gap:4px;color:var(--muted);font-size:12px}.avp-promo-badge{display:inline-block;font-size:10px;font-weight:900;letter-spacing:.08em;color:#56d7ff}.avp-promo-badge.azul{color:#70d8ff}.avp-promo-action{display:flex;align-items:center;justify-content:center;width:100%;margin-top:14px;padding:12px 14px;border-radius:11px;background:linear-gradient(135deg,#087cff,#5748ff);color:#fff;font-size:14px;font-weight:900;letter-spacing:.01em}.avp-promo-hint{display:block;margin-top:7px;text-align:center;color:var(--muted);font-size:10px}@media(min-width:700px){.avp-promotions-grid{grid-template-columns:repeat(2,minmax(0,1fr))}}@media(min-width:1000px){.avp-promotions-grid{grid-template-columns:repeat(3,minmax(0,1fr))}}
</style>`;
...
(()=>{
 ...
 async function loadPromotions(){
  ...
  try{
   ...
   box.innerHTML='<div class="avp-promotions-grid">'+items.map((p,i)=>'<a class="avp-promo '+(i===0?'best':'')+'" href="'+p.link+'" aria-label="Ver passagem de '+p.origin+' para '+p.destination+'"><span class="avp-promo-badge '+(p.azul?'azul':'')+'">'+(p.type==='alternative'?'DESTINO ALTERNATIVO':p.azul?'AZUL EM DESTAQUE':'OFERTA MONITORADA')+'</span><div class="avp-promo-route">'+p.origin+' → '+p.destination+'</div><div class="avp-promo-price">'+money(p.price)+'</div><div class="avp-promo-meta"><span>Ida: '+date(p.departure)+(p.returnDate?' • Volta: '+date(p.returnDate):'')+'</span><span>'+p.airline+' • '+(p.stops===0?'voo direto':p.stops+' escala(s)')+'</span><span>'+(p.adults||1)+' adulto(s)'+(p.children?' + '+p.children+' criança(s)':'')+'</span></div><span class="avp-promo-action">Ver passagem aérea →</span><small class="avp-promo-hint">Toque para abrir os detalhes e continuar a reserva</small></a>').join('')+'</div>';
