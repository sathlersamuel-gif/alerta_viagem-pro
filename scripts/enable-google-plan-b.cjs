...
if (!source.includes('const GOOGLE_PLAN_B_INTERVAL')) {
  source = source.replace(
    ...
    match => `${match}\nconst GOOGLE_PLAN_B_INTERVAL = 12 * 60 * 60 * 1000;`
  );
}
...
if (!source.includes('function googlePlanBTargets(')) {
  ...
  const rotation=Math.floor(Date.now()/(12*60*60*1000));
...
console.log('Plano B do Google Voos ativado com alertas rotativos a cada 12 horas.');
