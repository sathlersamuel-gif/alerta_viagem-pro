(() => {
  ...
  view.innerHTML = `
    <section class="panel glass">
      ...
      <p class="info-note">Cadastre a viagem uma vez. A IA usará essas informações para pesquisar oportunidades e enviar os avisos escolhidos.</p>
  ...
  style.textContent = `
    ...
    .managed-trip-card h4{margin:0 0 8px}.managed-meta{display:grid;gap:5px;color:#afc6d9;font-size:13px}.managed-card-actions{display:flex;gap:8px;margin-top:12px}.managed-card-actions button{flex:1}.managed-empty{padding:28px;text-align:center;color:#afc6d9}.managed-age-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(130px,1fr));gap:10px;margin-top:8px}
  ...
  function frequencyLabel(value) {
    return value === 'instant' ? 'quando melhorar' : value === 'daily' ? 'diário' : 'semanal';
  }
  ...
  function render() {
    ...
    list.innerHTML = items.map(item => `
      <article class="managed-trip-card" data-id="${item.id}">
        ...
        <div class="managed-meta">
          <span>Ida: ${fmt(item.departure)}${item.return ? ` • Volta: ${fmt(item.return)}` : ''}</span>
          ...
          <span>Avisos: ${channelLabel(item.channel)} • ${frequencyLabel(item.frequency)}</span>
    ...
    $$('.managed-trip-card', list).forEach(card => {
      ...
      $('[data-edit]', card).onclick = () => {
        ...
        renderChildAges(item.childAges || []);
  ...
  $('#managedTripForm').addEventListener('submit', event => {
    ...
    const item = {
      ...
      adults: Math.max(1, Number($('#managedAdults').value || 1)),
      children: Math.max(0, Number($('#managedChildren').value || 0)),
      childAges: $$('.managedChildAge').map(input => Number(input.value)),
      preference: $('#managedPreference').value,
      channel: $('#managedChannel').value,
      frequency: $('#managedFrequency').value,
      active: true,
      internalFlexDays: INTERNAL_FLEX_DAYS,
